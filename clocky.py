...
def main():
    ...
    while True:
        ...
        for i in range(len(pixels)):
            if pixels[i] == 'M':
                pixels[i] = "BGWR"[int(dt.now().minute / 15)]
            elif pixels[i] == 'S':
                pixels[i] = "BGWR"[int(dt.now().second / 15)]
            elif pixels[i] == '!':
                pixels[i] = 1
            elif pixels[i] == 'H':
                pixels[i] = "B"
        ...
        for i in filter(lambda _: bool(pixels[_]), range(len(pixels))):
            if type(pixels[i]) is int and pixels[i] == 1:
                set_pixel(i, 100, 0, 100, 0.1)
            elif pixels[i] == 'R':
                set_pixel(i, 200, 0, 0, 0.033)
            elif pixels[i] == 'G':
                set_pixel(i, 0, 200, 0, 0.033)
            elif pixels[i] == 'B':
                set_pixel(i, 0, 0, 200, 0.033)
            elif pixels[i] == 'W':
                set_pixel(i, 200, 200, 200, 0.033)

        # set second flash if that last pixel is free and it's normal mode
        if second_pix == 0 and pixels[-1] is None:
            if bool(dt.now().second % 2):
                set_pixel(len(pixels)-1, 20, 20, 20, 0.033)
